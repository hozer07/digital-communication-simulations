...
for nEN = 1:length(snr_db) % SNR POINTS
    ...
    while (nframe<max_nFrame) && (fErrs(nEN)<fErrLim)
        ...
        %%%%%%%%%%%%%CHANNEL %%%%%%%%%%%%%%%%%%%%%
        noise=1/sqrt(2)*[randn(1, nSymPerFrame)];
...
SNR = 10.^(snr_db./10);
bpsk_ber_SNR_exp = errs./nFrames/nSymPerFrame;
bpsk_ber_SNR_theory = qfunc(sqrt(2.*SNR));
...
ylim([1e-10 1]);
save bpsk.mat bpsk_ber_SNR_exp bpsk_ber_SNR_theory