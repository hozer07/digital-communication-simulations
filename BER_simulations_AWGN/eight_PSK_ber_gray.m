...
SNR = 10.^(snr_db./10);
epsk_gray_ber_SNR_exp = errs./nFrames/nBitsPerFrame;
epsk_gray_ber_SNR_theory = 2.*qfunc(sqrt(2.*SNR.*sin(pi/8).^2))./nBitPerSym;
semilogy(snr_db, epsk_gray_ber_SNR_exp, 'r-x');hold on;grid on;
semilogy(snr_db, epsk_gray_ber_SNR_theory, 'rs');
...
ylim([1e-10 1]);
save 8psk_gray.mat epsk_gray_ber_SNR_exp epsk_gray_ber_SNR_theory