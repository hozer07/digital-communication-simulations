...
for nEN = 1:length(snr_db) % SNR POINTS
    ...
    while (nframe<max_nFrame) && (fErrs(nEN)<fErrLim)
        ...
        %%%%%%%%%%%%%CHANNEL %%%%%%%%%%%%%%%%%%%%%
        noise=1/sqrt(2)*[randn(1, nSymPerFrame)+1i.*randn(1,nSymPerFrame)];
...
SNR = 10.^(snr_db./10);
bfsk_ber_SNR_exp = errs./nFrames/nSymPerFrame;
bfsk_ber_SNR_theory = qfunc(sqrt(SNR));
...
ylim([1e-10 1])
save bfsk.mat bfsk_ber_SNR_exp bfsk_ber_SNR_theory