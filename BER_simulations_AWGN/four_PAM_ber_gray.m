...
No = 1./10.^(snr_db./10);
dmin = 2/sqrt(5);
pam_gray_ber_SNR_exp = errs./nFrames/nBitsPerFrame;
pam_gray_ber_SNR_theory = 3/4.*qfunc(dmin./sqrt(2.*No));
semilogy(snr_db, pam_gray_ber_SNR_exp, 'r-x');hold on;grid on;
semilogy(snr_db, pam_gray_ber_SNR_theory, 'rs');
...
ylim([1e-10 1]);
save 4pam_gray.mat pam_gray_ber_SNR_exp pam_gray_ber_SNR_theory