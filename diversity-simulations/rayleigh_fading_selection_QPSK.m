...
for snr_index=1:length(SNR)
    ...
    while(err_found_count<err_found_lim && sim_var<sim_lim)
        ...
        received = received1.*(channel1>channel2)+received2.*(channel2>channel1);
    ...
    while(err_found_count<err_found_lim && sim_var<sim_lim)
        ...
        received = received1.*(channel1>channel2 & channel1>channel3)+...
            received2.*(channel2>channel1 & channel2>channel3)+...
            received3.*(channel3>channel1 & channel3>channel2);
        [~,detected] = min(abs(repmat(received,1,4)-conj(qpsk_syms')),[],2);
        err = sum(detected~=qpsk_data,'all');
    ...
    while(err_found_count<err_found_lim && sim_var<sim_lim)
        ...
        received = received1.*(channel1>channel2 & channel1>channel3 & channel1>channel4)+...
            received2.*(channel2>channel1 & channel2>channel3 & channel2>channel4)+...
            received3.*(channel3>channel1 & channel3>channel2 & channel3>channel4)+...
            received4.*(channel4>channel1 & channel4>channel2 & channel4>channel3);
        [~,detected] = min(abs(repmat(received,1,4)-conj(qpsk_syms')),[],2);
        err = sum(detected~=qpsk_data,'all');
...
semilogy(SNR,fading_err_store(4,:),'--^','MarkerSize',7);ylim([1e-4 1]);
xlabel('SNR (dB)');ylabel('BER');
title('Rayleigh fading, QPSK, Selection combining');
