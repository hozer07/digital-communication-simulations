...
for snr_index=1:length(SNR)
    ...
    while(err_found_count<err_found_lim && sim_var<sim_lim)
        ...
        channel = abs(sqrt(0.125).*(randn(1,symbol_num)+1i.*randn(1,symbol_num))+sqrt(0.375).*(1+1i));
    ...
    while(err_found_count<err_found_lim && sim_var<sim_lim)
        ...
        channel = abs(sqrt(0.125).*(randn(symbol_num,1)+1i.*randn(symbol_num,1))+sqrt(0.375).*(1+1i));
    ...
    while(err_found_count<err_found_lim && sim_var<sim_lim)
        ...
        channel = abs(sqrt(0.5).*(randn(symbol_num,1)+1i.*randn(symbol_num,1))+sqrt(0.375).*(1+1i));
...
step_size = pi/20000;
interval = step_size:step_size:pi/2;
mgf = @(beta,theta,SNR_dec) 4./(4+beta./sin(theta).^2.*SNR_dec./2).*exp(-3*beta*SNR_dec./sin(theta).^2./2./(4+beta./sin(theta).^2.*SNR_dec./2));
for i=1:length(SNR)
    SNR_dec = 10^(SNR(i)/10);
    theoretical_bpsk(i) = 1/pi.*sum(mgf(2,interval,SNR_dec)).*step_size;
    theoretical_qpsk(i) = 1/pi.*sum(mgf(1,interval,SNR_dec)).*step_size;
    theoretical_8psk(i) = 2/3/pi.*sum(mgf(sin(pi/8)^2*2,interval,SNR_dec)).*step_size;
...
semilogy(SNR,theoretical_8psk,'o','MarkerSize',8);
semilogy(SNR,theoretical_16qam,'d','MarkerSize',8);
title('Rician Fading K=3');
xlabel('E_s/N_0 (dB)');
ylabel('BER');
