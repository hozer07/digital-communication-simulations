...
for snr_index=1:length(SNR)
    ...
    while(err_found_count<err_found_lim && sim_var<sim_lim)
        ...
        channel = abs(sqrt(0.5).*(randn(1,symbol_num)+1i.*randn(1,symbol_num)));
...
for i=1:length(SNR)
    SNR_dec = 10^(SNR(i)/10);
    theoretical_bpsk(i) = 1/pi.*sum(1./(1+SNR_dec./sin(interval).^2)).*step_size;
    theoretical_qpsk(i) = 1/pi.*sum(1./(1+SNR_dec./2./sin(interval).^2)).*step_size;
    theoretical_8psk(i) = 2/3/pi.*sum(1./(1+sin(pi/8).^2.*SNR_dec./sin(interval).^2)).*step_size;
    theoretical_16qam(i) = 1/pi.*sum(1./(1+SNR_dec./10./sin(interval).^2)).*step_size;
end
